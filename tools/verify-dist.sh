...
# ${N} 의 중괄호는 장식이 아니다. "$N개" 처럼 변수 뒤에 한글이 바로 붙으면,
# 셸이 변수명의 끝을 판정할 때 로케일의 문자 분류를 쓰기 때문에 ko_KR.UTF-8
# 같은 환경에서 한글의 선행 바이트를 변수명의 일부로 삼아 "unbound variable"
# 로 죽는다. 개발 기계와 CI 에서는 통과하고 사용자 터미널에서만 깨졌다.
[ "$N" -ge 10 ] && say "개발자 표기 (${N}개 파일)" "통과" || bad "개발자 표기" "${N}개 파일뿐"
